...
export default function RealtimeCharts() {
  const [counts, setCounts] = useState<Counts>({ properties: 0, submissions: 0, inquiries: 0, verified: 0 });
  ...
  const fetchCounts = async () => {
    try {
      ...
      console.warn("Failed to fetch realtime counts", err);
    }
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("realtime-admin-charts")
      .on("postgres_changes", { event: "*", schema: "public", table: "properties" }, () => fetchCounts())
      .on("postgres_changes", { event: "*", schema: "public", table: "property_submissions" }, () => fetchCounts())
      .on("postgres_changes", { event: "*", schema: "public", table: "buyer_inquiries" }, () => fetchCounts())
      .subscribe();
